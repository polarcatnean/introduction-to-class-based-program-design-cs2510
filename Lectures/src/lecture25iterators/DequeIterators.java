...
class DequeForwardIterator<T> implements Iterator<T> {
  ANode<T> node;  // starting node
  DequeForwardIterator(ANode<T> node) { this.node = node; }
  
  /* for loop would automatically call iterator() on deque
   * for (T item : myDeque) {
  ...
  public boolean hasNext() {
    return !this.node.isSentinel();
  }

  @Override
  public T next() {
    // produce this item and move to next node
    T currentItem = this.node.getData();
...
class DequeReverseIterator<T> implements Iterator<T> {
  ...
  /* Iterator<T> revIter = deque.reverseIterator();
  ...
  @Override
  public T next() {
    // produce this item and move to previous node
    T currentItem = this.node.getData();
...
class Deque<T> implements Iterable<T>{
  ...
  public String details() {
    return "Deque data: " + this.header.details();
  }
...
class Sentinel<T> extends ANode<T> {
  ...
  public String details() {
    ...
    return "Sentinel, " + this.next.details();
  }
...
class Node<T> extends ANode<T> {
  ...
  protected String details() {
    if (this.next.isSentinel()) {
      return this.data.toString() + ", " + "Sentinel end)";
    }
    return this.data.toString() + ", " + this.next.details();
  }
}
...
class DequeExamples {
  ...
  void testAddAtTail(Tester t) {
    init();
    deque1.addAtTail(0);
    System.out.println(deque1.details());  // 0
    deque1.removeFromHead();
    System.out.println(deque1.details());  // empty
    deque1.addAtTail(1);
    deque1.addAtTail(2);
    System.out.println(deque1.details());  // 1, 2
    
    deque2.addAtTail("Nean");
    System.out.println(deque2.details());
  }
}