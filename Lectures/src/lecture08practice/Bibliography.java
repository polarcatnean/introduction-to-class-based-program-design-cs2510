...
interface IBiblio {
  ...
  // make a list of biblio, wiki article removed & duplicates removed
  IBiblio makeBiblio();
  IBiblio removeWiki();
  
  // produce a list of string 
  ILoString formatBiblio();
  // alphabetised, removed duplicates, entries formatted as a list of string
  ILoString formatAll();
...
class MtB implements IBiblio {
  ...
  public ILoString formatBiblio() {
    return new MtLoString();
  }

  public ILoString formatAll() {
    return new MtLoString();
  }
}
...
class ConsB implements IBiblio {
  ...
  public ILoString formatBiblio() {
    return new ConsLoString(this.first.format(), this.rest.formatBiblio());
  }

  public ILoString formatAll() {
    return this.makeBiblio().sortByLastName().formatBiblio();
  }
}
...
class ExampleBiblio {
  ...
  Author jeremy = new Author("Jeremy", "Kun");
  ...
  boolean testFormatBiblio(Tester t) {
    return t.checkExpect(empty.formatBiblio(), new MtLoString())
        && t.checkExpect(bib1.formatBiblio(), new ConsLoString("Vinge, Vernor. \"Across Realtime\".",  new MtLoString()))
        && t.checkExpect(wiki1.makeBiblio().formatBiblio(), 
            new ConsLoString("Carter, Constance. \"John Gould Stephenson: Largely Known and Much Liked\".",  
                new ConsLoString("Cole, John Y. \"The Main Building of the Library of Congress: A Chronology, 1871–1965\".", 
                    new ConsLoString("Vinge, Vernor. \"Across Realtime\".",  new MtLoString()))));
  }
  
  boolean testFormatAll(Tester t) {
    return t.checkExpect(empty.formatAll(), new MtLoString())
        && t.checkExpect(book6.makeBiblio().formatAll(),
            new ConsLoString("Hamilton, Peter F. \"Mindstar Rising\".",  
                new ConsLoString("Hamilton, Peter F. \"The Reality Dysfunction\".", 
                    new ConsLoString("Kun, Jeremy. \"A Programmer's Introduction to Mathematics\".",  
                        new ConsLoString("Stross, Charles. \"Halting State\"." , 
                            new ConsLoString("Vinge, Vernor. \"Across Realtime\".", new MtLoString()))))));
  }
}