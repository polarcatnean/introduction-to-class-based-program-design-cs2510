...
interface ILoInt {
  ...
  boolean variantC();
  boolean variantCHelper(boolean a, boolean b, boolean c);
}
...
class MtLoInt implements ILoInt {
  ...
  public boolean variantCHelper(boolean a, boolean b, boolean c) {
    return a && b && c;
  }
...
class ConsLoInt implements ILoInt {
  ...
  public boolean variantBHelper(boolean condA, boolean condB, boolean condC) {
    ...
    if (!newA) {
      newA = this.isEven(first);
    }
    
    if (!newB) {
      newB = this.isOddandPos(first);
    }
    
    if (!newC) {
      newC = this.is5to10(first);
    }

    // Recursively check the rest of the list
    return this.rest.variantBHelper(newA, condB, condC)  
        || this.rest.variantBHelper(condA, newB, condC)   
        || this.rest.variantBHelper(condA, condB, newC);  
  }
  ...
  public boolean variantCHelper(boolean a, boolean b, boolean c) {
    ...
    if (this.isExtra(first, a, b, c)) {
      return false;
    }
    
    boolean branchA = false;
    boolean branchB = false;
    boolean branchC = false;
    
    if (this.isEven(first) && !a) {
      branchA = this.rest.variantCHelper(true, b, c);
    }
    
    if (this.isOddandPos(first) && !b) {
      branchB = this.rest.variantCHelper(a, true, c);
    }
    
    if (this.is5to10(first) && !c) {
      branchC = this.rest.variantCHelper(a, b, true);
    }

    // Recursively check the rest of the list
    return branchA || branchB || branchC; 
  }
...
class ExampleLoInt {
  ...
  ILoInt list4s = new ConsLoInt(6, new ConsLoInt(5, new ConsLoInt(8 ,new ConsLoInt(42 ,empty))));
  ILoInt list5 = new ConsLoInt(10, new ConsLoInt(5, new ConsLoInt(2 ,empty)));
  ILoInt list6 = new ConsLoInt(10, new ConsLoInt(5, new ConsLoInt(2 ,new ConsLoInt(-5, new ConsLoInt (-7, new ConsLoInt(-3, empty))))));
  ...
  boolean testVariantB(Tester t) {
    return t.checkExpect(empty.variantB(), false)
        ...
        && t.checkExpect(list4s.variantB(), true)
        && t.checkExpect(list5.variantB(), true)
        && t.checkExpect(list6.variantB(), true);
  }
  ...
  boolean testVariantC(Tester t) {
    return t.checkExpect(empty.variantC(), false)
        ...
        && t.checkExpect(list6.variantC(), false);
  }
}
