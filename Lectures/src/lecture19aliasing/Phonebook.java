...
class SamePerson implements IPred<Person> {
  Person p;
  SamePerson(Person p) { this.p = p; }
  
  public boolean apply(Person other) {
    return this.p.name.equals(other.name) && this.p.phone == other.phone;
  }
}

class Contains implements IPred<IList<Person>> {
  String name;
  Contains(String name) { this.name = name; }
  
  public boolean apply(IList<Person> list) {
...
class ChangePhone implements IFunc<Person, Void> {
  int newNum;
  ChangePhone(int newNum) { this.newNum = newNum; }
...
interface IList<T> {
  ...
  Void findAndModify(IPred<T> whichOne, IFunc<T,Void> whatToDo);
}
...
class MtList<T> implements IList<T> {
  ...
  public Void findAndModify(IPred<T> whichOne, IFunc<T, Void> whatToDo) {
    return null;
  }
}
...
class ConsList<T> implements IList<T> {
  ...
  public Void findAndModify(IPred<T> whichOne, IFunc<T, Void> whatToDo) {
    if (whichOne.apply(this.first)) {
      ...
      this.rest.findAndModify(whichOne, whatToDo);
    }
    return null;
  }
}
...
//  }
  ...
  void testFindPhoneNum(Tester t) {
    this.initData();
    t.checkExpect(this.friends.find(new SameName("Gail")).phone, 9345);
    t.checkExpect(this.work.find(new SameName("Zelda")), null);
    t.checkExpect(this.friends.find(new SameName("Frank")), this.family.find(new SameName("Frank")));
  }
  
  void testChangePhoneNum(Tester t) {
    this.initData();
    t.checkExpect(this.friends.find(new SameName("Anne")).phone, 1234);
    t.checkExpect(this.friends.find(new SameName("Anne")), this.family.find(new SameName("Anne")));
    t.checkExpect(this.anne.phone, 1234);

    new ChangePhone(1111).apply(anne);
    // Try to test ALL side effects
    t.checkExpect(this.friends.find(new SameName("Anne")).phone, 1111);  
    t.checkExpect(this.friends.find(new SameName("Anne")), this.family.find(new SameName("Anne")));  
