...
class Graph {
  ...
  public List<Edge> kruskal() {    
    ...
    while (hasManyTrees(representatives)) {
      ...
      String ultimateRepOfFrom = find(representatives, nextEdge.from.name);
      String ultimateRepOfTo = find(representatives, nextEdge.to.name);
      
      if (ultimateRepOfFrom.equals(ultimateRepOfTo)) {  // already connected/in the same tree
        continue;
      }
      else {
        edgesInTree.add(nextEdge);
        union(representatives, ultimateRepOfFrom, ultimateRepOfTo);  // set repOfFrom -> repOfTo
      }
    }
    return edgesInTree;
  }

  // EFFECT: set rep1's rep to rep2
  private void union(HashMap<String, String> representatives, String rep1, String rep2) {
    representatives.put(rep1, rep2);
  }

  // produce the ultimate rep of the vertex, given a vertex's name
