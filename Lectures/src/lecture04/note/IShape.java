...
//  }
  ...
  public double distance(IPoint p) {
    if (p instanceof CartPt) {
      CartPt other = (CartPt) p;
      return Math.sqrt(Math.pow((this.x - (other.x)), 2) + Math.pow((this.y - other.y), 2));
    }
    else if (p instanceof PolarPt) {
      return this.distance(((PolarPt) p).toCartesian());
    }
    throw new IllegalArgumentException("Unsupported point type");
  }
  
  public PolarPt toPolar() {
    return new PolarPt(Math.sqrt(Math.pow(this.x, 2) + Math.pow(this.y, 2)),
                       Math.toDegrees(Math.atan2(this.y, this.x)));
  }
}
...
//  public double distance(PolarPt p) {
//    return Math.sqrt(Math.pow(this.r, 2) + Math.pow(p.r, 2) 
//                     - (2 * (this.r * p.r * Math.cos(this.theta - p.theta))));
//   }

  public double distance(IPoint p) {
    if (p instanceof PolarPt) {
      PolarPt other = (PolarPt) p;
      return Math.sqrt(Math.pow(this.r, 2) + Math.pow(other.r, 2) - 
               (2 * (this.r * other.r * Math.cos(this.theta - other.theta))));
    } 
    else if (p instanceof CartPt) {
      return this.distance(((CartPt) p).toPolar());
    }
    throw new IllegalArgumentException("Unsupported point type");
  }
  
  public CartPt toCartesian() {
    return new CartPt(this.r * Math.cos(Math.toRadians(this.theta)), 
                      this.r * Math.sin(Math.toRadians(this.theta)));
  }
}
...
class ExamplesIShape {
  ...
  boolean testForCombo(Tester t) {
    return t.checkInexact(combo1.area(), c2.area() + s4.area(), 0.01)
        ...
        && t.checkExpect(combo1.contains(new CartPt(50, 50)), true);
  }
}