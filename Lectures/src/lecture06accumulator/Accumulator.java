...
interface ILoString {
  ...
  ILoString AppendAccHelp(ILoString list, ILoString acc);  // a result-so-far accumulator
}
...
class MtLoString implements ILoString {
  ...
  public ILoString AppendAccHelp(ILoString list, ILoString acc) {
    return acc.append(list);
  };
}
...
class ConsLoString implements ILoString {
  ...
  public ILoString appendAcc(ILoString list) {
    // TODO check
    return this.AppendAccHelp(list, new MtLoString());  // acc - starts with empty list
  }

  public ILoString AppendAccHelp(ILoString list, ILoString acc) {
    return acc.append(this).append(list);
  }
}
...
interface IAT {
  ...
  IAT youngestAncAtGen(int gen);   // <- here int gen is a context-preserving "accumulator"
}
...
class ExamplesIAT {
  ...
  ILoString list2 = new ConsLoString("a", new ConsLoString("b", new ConsLoString("c", empty)));
  ILoString list3 = new ConsLoString("e", new ConsLoString("f", new MtLoString()));
  ILoString list4 = new ConsLoString("a", new ConsLoString("b", new ConsLoString("c",  new ConsLoString("e", new ConsLoString("f", new MtLoString())))));
  ...
  boolean testAppend(Tester t) {
    return t.checkExpect(new MtLoString().append(list1), list1)
        ...
        && t.checkExpect(list2.append(list3), list4);
  }
  
  
  boolean testAppendAcc(Tester t) {
    return t.checkExpect(new MtLoString().appendAcc(list1), list1)
        && t.checkExpect(list1.appendAcc(empty), list1)
        && t.checkExpect(list1.appendAcc(list1), 
            new ConsLoString("David", new ConsLoString("Edward",
                new ConsLoString("David", new ConsLoString("Edward", empty)))))
        && t.checkExpect(list2.appendAcc(list3), list4);
  }
