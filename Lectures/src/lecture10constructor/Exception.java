...
class ExamplesDates {
  ...
  boolean testCheckConstructorException(Tester t) {
    return t.checkConstructorException(new IllegalArgumentException("Invalid year: -30"), "constructor.Date", -30, 33, 23)
        && t.checkConstructorException(new IllegalArgumentException("Invalid year: 1400"), "constructor.Date", 1400, 33, 23)
        && t.checkConstructorException(new IllegalArgumentException("Invalid month: -1"), "constructor.Date", 2020, -1, 23)
        && t.checkConstructorException(new IllegalArgumentException("Invalid month: 13"), "constructor.Date", 2020, 13, 32)
        && t.checkConstructorException(new IllegalArgumentException("Invalid day: 0"), "constructor.Date", 2020, 1, 0)
        && t.checkConstructorException(new IllegalArgumentException("Invalid day: 32"), "constructor.Date", 2020, 1, 32);
  }
  
  Utils u = new Utils();
  
  boolean testCheckRangeMethodException(Tester t) {
    return t.checkException(new IllegalArgumentException("Invalid value"), u, "checkRange", 3000, 1500, 2100, "Invalid value")
        && t.checkException(new IllegalArgumentException("Invalid value"), u, "checkRange", 0, 1, 12, "Invalid value")
        && t.checkNoException(u, "checkRange", 5, 1, 31, "Invalid value");
  }
