...
class CircleGame extends World {
  ...
  int RANDOM_RANGE = 20;
  WorldScene MTScene = new WorldScene(WIDTH, HEIGHT);
...
//      rand.nextInt(RANDOM_RANGE) - RANDOM_RANGE/2); 
  ...
  CircleGame(int remaining) {
    this(new MtLoCircle(), remaining);
  }

  // METHODS 
  public WorldScene makeScene() {
    // return the WorldScene to be shown on each clock tick
    return circles.placeAll(MTScene).placeImageXY(this.showRemaining(), WIDTH - 60 , 20);
  }
  ...
  public WorldScene makeEndScene() {
    ...
    return endScene.placeImageXY(new TextImage("Game Over", 24, FontStyle.BOLD, Color.red), 
        WIDTH/2, HEIGHT/2);
