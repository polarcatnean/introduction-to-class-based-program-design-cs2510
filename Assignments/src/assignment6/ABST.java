...
abstract class ABST<T> {
  ...
  protected abstract T getLeftMostHelper(Node<T> prevNode);
  
  // produce the tree containing all but the leftmost item of this tree.
  public abstract ABST<T> getRight();
  protected abstract ABST<T> getRightHelper(Node<T> prevNode);
  ...
  public boolean sameDataLeaf(Leaf<T> leaf) { return false; }
  public boolean sameDataNode(Node<T> other) { return false; }
  protected abstract boolean allPresent(ABST<T> other);
...
class Leaf<T> extends ABST<T> {
  ...
  protected T getLeftMostHelper(Node<T> prevNode) {
  ...
  protected ABST<T> getRightHelper(Node<T> prevNode) {
  ...
  public boolean sameData(ABST<T> other) {
    return other.sameDataLeaf(this); 
  }
  
  @Override
  public boolean sameDataLeaf(Leaf<T> leaf) {
    return true;  // if other is leaf, it is the same
  }
  
  protected boolean allPresent(ABST<T> other) {
    return true;
  }
...
//                                    this.removeLeftNode());     // rsf = all tree except the left node on the same level
  ...
  protected ABST<T> getRightHelper(Node<T> prevNode) {
    return new Node<T>(prevNode.order, prevNode.data, this.left.getRightHelper(this), prevNode.right);
//    if (this.left.isLeaf()) {
//      return rightTree;
//    }
...
//                                    rightTree.addLeftMostNode(leftNode));
  ...
  @Override
  public boolean sameDataNode(Node<T> other) {
   // other tree contains all data in this tree AND TODO vice versa
    return this.allPresent(other) && other.allPresent(this);
  }
  
  //other tree contains all data in this tree
  protected boolean allPresent(ABST<T> other) {
    return other.present(this.data) && this.left.allPresent(other) && this.right.allPresent(other);
  }

  public IList<T> buildList() {
    // keep getting leftmost, reduce the problem by using getRight()
...
class ExamplesBSTBooks {
  ...
  boolean testLeftMost(Tester t) {
    return t.checkException(new RuntimeException("No leftmost item of an empty tree"), 
        ...
        && t.checkExpect(tree5.getLeftMost(), 1);
  }
  ...
  boolean testGetRight(Tester t) {
    return t.checkException(new RuntimeException("No right of an empty tree"), 
        ...
        && t.checkExpect(tree5.getRight(),
            new Node<>(asc, 2, new Leaf<>(asc),
                new Node<>(asc, 3, new Leaf<>(asc),
                    new Node<>(asc, 4, new Leaf<>(asc),
                        new Node<>(asc, 5, new Leaf<>(asc), new Leaf<>(asc))))))
        && t.checkExpect(tree6.getRight(), tree6r)
        && t.checkExpect(tree1.getRight(), new Leaf<Integer>(asc));
  }
