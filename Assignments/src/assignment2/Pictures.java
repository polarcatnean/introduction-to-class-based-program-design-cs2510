...
interface IPicture {
  ...
  // In more detail: invoking pictureRecipe on a Combo produces its name if the
  // given depth is less than or equal to 0, and the formula of its mixture (at
...
class Overlay implements IOperation {
  ...
  public String pictureRecipe(int depth) {
    return "overlay(" + topPic.pictureRecipe(depth) + ", " + bottomPic.pictureRecipe(depth) + ")";
  }
}
...
class ExamplesPicture {
  ...
  IPicture squareAndCircle = new Combo("square and circle", new Beside(square, circle));
  ...
  boolean testMirror(Tester t) {
    return t.checkExpect(circle.mirror(), circle)                   // no operation
        ...
        && t.checkExpect(circSqCircTri.mirror(),          // depth 2, with Beside twice
  ...
  // IPicture doubledSquareOnCircle = 
  // new Combo("doubled square on circle", new Beside(this.squareOnCircle, this.squareOnCircle));
}