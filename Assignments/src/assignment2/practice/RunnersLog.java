...
interface ILog {
  ...
  // create a sorted by pace version of this log (asc); pace = min/km
  ILog sortByPace();
  ILog insertPace(Entry e);
}
...
class MtLog implements ILog {
  ...
  public ILog sortByPace() {
    return this;
  }

  public ILog insertPace(Entry e) {
    return new ConsLog(e, this);
  }
}
...
class ConsLog implements ILog {
  ...
  public ILog insertDist(Entry e) {
    if (e.distance > this.first.distance) {
      ...
      return new ConsLog(this.first, this.rest.insertDist(e));
    }
  }

  public ILog sortByPace() {
    return this.rest.sortByPace().insertPace(this.first);
  }

  public ILog insertPace(Entry e) {
    if (e.pace() > this.first.pace() ) {
      return new ConsLog(e, this);
    }
    else {
      return new ConsLog(this.first, this.rest.insertPace(e));
    }
  }
...
class LogExamples {
  ...
  boolean testSortByDist(Tester t) {
    return t.checkExpect(l1.sortByDist(), l1)   // empty log
        ...
        && t.checkExpect(l4.sortByDist(), new ConsLog(e3, new ConsLog(e1, new ConsLog(e2, l1))));
  }
  ...
  boolean testSortByPace(Tester t) {
    return t.checkExpect(null, null);
  }
  
  boolean testInsertPace(Tester t) {
    return t.checkExpect(null, null);
  }
  
  boolean testPace(Tester t) {
    return t.checkInexact(e1.pace(), 25.0/5.0, 0.01)
        && t.checkInexact(e2.pace(), 24.0/3.0, 0.01)
        && t.checkInexact(e3.pace(), 156.0/26.0, 0.01);
  }
}